  ...
  return (
    <div className="w-full bg-[#bae6fd] px-4 py-2 sm:px-6 md:px-12 md:py-4 flex flex-col sm:flex-row justify-between items-center z-50 border-b-2 border-white/50 shadow-sm gap-3 sm:gap-0 sticky top-0">
      ...
      <div className="flex items-center gap-2 sm:gap-3 lg:gap-4 w-full sm:w-auto justify-end">
        <button onClick={() => {}} className="bg-white hover:bg-gray-100 text-blue-600 rounded-full w-8 h-8 md:w-10 md:h-10 flex items-center justify-center shadow-sm transition-all focus:outline-none focus:ring-2 focus:ring-blue-300">
        ...
        <button onClick={() => navigate("/login")} className="bg-white hover:bg-gray-50 text-black font-bold py-2 px-4 md:px-6 rounded-full shadow-md transition-all text-sm md:text-base border border-gray-200 whitespace-nowrap">Masuk Admin</button>
      </div>
    </div>
  );
}
