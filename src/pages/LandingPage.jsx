...
const LandingPage = () => {
  ...
  return (
    <div className="flex w-screen h-screen overflow-hidden bg-gray-200">
      ...
      <div
        className={`absolute bottom-6 left-3 z-[1000] w-[calc(100vw-24px)] md:w-72 transition-all duration-300 ease-out ${
      ...
      <RightSidebar isOpen={isSidebarOpen} setIsOpen={setIsSidebarOpen} desaName="SIDOARJO" themeName="PETA KECAMATAN">
        ...
        <div className="bg-gray-50 rounded-xl p-3">
          <div className="font-bold text-gray-700 text-xs mb-2 pb-1.5 border-b border-gray-200">
            {mapMode === "kepadatan" ? "Legenda Kepadatan" : "Legenda Rasio Kelamin"}
          </div>
          <div className="text-xs space-y-1.5">
            {mapMode === "kepadatan" && [
              ...
              { color: "#bfdbfe", label: "< 500" },
            ].map(({ color, label }) => (
              <div key={label} className="flex items-center gap-2">
                <span className="w-3.5 h-3.5 rounded shrink-0" style={{ background: color }}/>
                <span className="text-gray-600">{label}</span>
              </div>
            ))}
            {mapMode === "rasio" && [
              { color: "#1e3a8a", label: ">105 Dominan Laki-laki" },
              { color: "#3b82f6", label: "102–105 Lebih Laki-laki" },
              { color: "#9ca3af", label: "98–102 Seimbang" },
              { color: "#ec4899", label: "95–98 Lebih Perempuan" },
              { color: "#be185d", label: "<95 Dominan Perempuan" },
            ].map(({ color, label }) => (
              <div key={label} className="flex items-center gap-2">
                <span className="w-3.5 h-3.5 rounded shrink-0" style={{ background: color }}/>
                <span className="text-gray-600">{label}</span>
        ...
        {sidoarjoAgregat && (
          <div className="bg-white border border-gray-100 rounded-xl p-3">
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">Demografi Sidoarjo</p>
        ...
        {selectedKecamatan && (
          <div className="mt-4 shrink-0">
            <AIInsightBox
              desaName=""
              featureName={`Kecamatan ${selectedKecamatan}`}
