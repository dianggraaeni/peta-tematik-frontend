...
const TopEllipse = () => {
  return (
    <div className="absolute z-0 flex items-center justify-center top-[calc(50%-260px)] md:top-[5%] md:transform md:-translate-x-[224px] md:translate-y-[110%] 2xl:translate-y-[75%]">
      <div className="size-[100px] rounded-full bg-gradient-to-r from-pblue to-[#eaffdb]"></div>
    </div>
  );
};
...
const LoginSidokepung = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        localStorage.setItem("token-sidokepung", response.data.token);
        localStorage.setItem("username", username);
        setLoading(false);
        navigate("/admin-sidokepung");
      }
    } catch (error) {
      if (error.response) {
  ...
  return (
    <div className="relative flex items-center justify-center min-h-screen bg-[#ADD8E6]">
      <TopEllipse />
      <BottomEllipse />
      <div className="relative z-10 flex flex-col items-center justify-center w-full max-w-sm p-8 m-5 bg-white shadow-md rounded-xl md:max-w-md">
        <div className="flex flex-col items-center mb-8">
          ...
          />
          <p className="text-lg italic font-bold font-inter text-black md:text-xl">
            KETENAGAKERJAAN
          </p>
        ...
        >
          ...
          <div className="relative">
            ...
            >
              {passwordType === "password" ? (
                <AiFillEye className="text-[#1E90FF]" />
              ) : (
                <AiFillEyeInvisible className="text-pblue" />
              )}
            </button>
          </div>
          {error && (
          ...
          <div className="flex justify-center">
            <Button
              type="submit"
              className="w-full my-2 font-bold text-white font-nunito bg-[#1E90FF]"
              disabled={loading}
            >
              {loading ? (
...
export default LoginSidokepung;
